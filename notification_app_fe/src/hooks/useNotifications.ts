...
import type { CategoryFilter, Notification, NotificationsState } from "../types";
...
export function useNotifications(
  ...
  // Apply category filter and log the filtering event.
  const notifications: Notification[] = (() => {
    if (categoryFilter === "All") {
      Log(
        ...
        "debug",
        "hook",
        `Notification filter: All (${allNotifications.length} total).`
      );
    ...
    Log(
      ...
      "debug",
      "hook",
      `Notification filter: ${categoryFilter} → ${filtered.length} items.`
    );
    return filtered;
  })();
