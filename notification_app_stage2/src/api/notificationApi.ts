...
import type { Notification, NotificationsResponse, RawNotification, NotificationCategory } from "../types";

export interface FetchNotificationsOptions {
  limit?: number;
  page?: number;
  notification_type?: NotificationCategory | "All";
}
...
/**
 * Fetches notifications from the evaluation service and returns them
 * as normalised Notification objects. Supports pagination and filtering.
 *
 * The Bearer token is attached automatically by the base apiRequest client.
 *
 * @param options - Pagination and filtering options (limit, page, notification_type).
 ...
 */
export async function fetchNotifications(options?: FetchNotificationsOptions): Promise<Notification[]> {
  Log("frontend", "info", "api", "Fetching notifications from evaluation service.");

  const url = new URL(ENDPOINTS.NOTIFICATIONS, window.location.origin);
  if (options) {
    ...
    if (options.notification_type && options.notification_type !== "All") {
      url.searchParams.append("notification_type", options.notification_type);
    }
  }
